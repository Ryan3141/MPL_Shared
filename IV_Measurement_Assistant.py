...
class IV_Controller( QtCore.QObject ):
	...
	Bias_Is_Set = QtCore.pyqtSignal( float, float )
	...
	def Initialize_Connection( self, which_machine ):
		...
		supported_devices = { "Keysight" : ( 'GPIB::23::INSTR', self.Voltage_Sweep_Keysight ), # Keysight B2962A's address on GPIB connection with IEEE-446 protocol
		                      "Keithly" : ('GPIB::26::INSTR', self.Voltage_Sweep_Keithly ) } # Keithly 236's address on GPIB connection with IEEE-446 protocol
	...
	def Voltage_Sweep_Dummy( self, input_start, input_end, input_step, time_interval ):
		x_values = np.arange( input_start, input_end + input_step, input_step, time_interval )
	...
	def Voltage_Sweep_Keithly( self, input_start, input_end, input_step, time_interval ):
		...
		Keithly.write( "O1X" ) # Change to remote sensing (4 point probe) O1 or 2 point probe O0
		...
		Keithly.timeout = 10 * 120000 # The measurement may take up to 1200 seconds
